 ...
 権限制御として、一般ユーザーは「使用済み」への変更のみ可能とし、「未使用」への復元は Admin のみが行える設計にしています。 
 ...
 また、チケット状態をユーザーが自由に復元できる設計では不正利用のリスクが発生するため、

 - 一般ユーザーは「使用済み」への変更のみ可能 
 - 「未使用」への復元は Admin のみ可能 
...
また、UI 側では JavaScript を利用して状態管理を行い、オフライン時でも画面上で状態変更を継続できる構成としています。
